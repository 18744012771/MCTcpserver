...
void TcpSocket::sentData(const QByteArray &data, const int id)
{
     ...
     qDebug()<<"sentData"<<QThread::currentThread();
     qDebug()<<"sentData-->"<<data;
...
void TcpSocket::disConTcp(int i)
{
    ...
    qDebug()<<"disConTcp"<<QThread::currentThread();
    if (i == socketID)
    {
...
{
    qDebug()<<"readData()"<<QThread::currentThread();
//    datas.append(this->readAll());
    QByteArray str;
    //while(this->bytesAvailable()>0)
        str=this->readAll();
    auto data  = handleData(str,this->peerAddress().toString(),this->peerPort());
    qDebug() << data;
    //this->write(data);

    emit receiveData(this->peerPort(),this->peerAddress().toString(),socketID,str);
